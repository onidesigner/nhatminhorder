            ...
            <div class="col-md-6 col-md-offset-3">
                <div class="widget-head-color-box navy-bg p-lg text-center">
                    ...
                    <div class="">
                        ...
                        <h3>Có {{count($orders)}} đơn hàng đặt cọc thành công</h3>
                    </div>
                </div>
                @if(count($orders))
                    @foreach($orders as $order)
                        <div class="widget-text-box">
                            <img style="float: left; margin-right: 10px;" src="{{ $order->avatar }}" width="50px" alt="">
                            <h4 class="media-heading"><a href="{{ url('don-hang', $order->id)  }}" title="{{$order->code}}">{{$order->code}}</a></h4>
                            <p>Đã đặt cọc.</p>
                        </div>
                    @endforeach
                @endif
