...
//    }
    ...
    {
        //before save code
        $this->calculate_weight = $this->getWeightCalFee();

        $saved = parent::save($options); // TODO: Change the autogenerated stub

        //cap nhat can nang kien hang len don hang
        $order = Order::find($this->order_id);
        if($order instanceof Order){
            $order->package_weight_original = $this->getWeightWithType('weight');
            $order->package_weight_converted = $this->getWeightWithType('converted_weight');
            $order->package_weight_calculate = $this->getWeightWithType('calculate_weight');
            $order->save();
        }
    ...
    /**
     * @author vanhs
     * @desc Tong can nang tinh, quy doi, tinh phi cua don hang
     * @param string $weight_type
     * @param null $order_id
     * @return int
     */
    public function getWeightWithType($weight_type = 'weight', $order_id = null){
        $weight = 0;
        ...
        $sql_where = "";
        if($weight_type == 'calculate_weight'){
            $sql_where .= " and is_done = 1 ";
        }
        $query = DB::select("
            select sum({$weight_type}) as weight
            from packages 
            where order_id = {$order_id} 
            and is_deleted = 0 {$sql_where};
        ");
        if($query){
