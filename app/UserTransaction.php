...
//    }
    ...
    /**
     * @author vanhs
     * @desc Lay tong tien van chuyen khach da thanh toan tren don hang
     * @param $order_id
     * @return int
     */
    public static function getOrderShippingAmountPaid($order_id){
        $amount = 0;
        $query = DB::select(sprintf("
        select sum(amount) as amount 
        from `user_transaction` 
        where transaction_type = '%s' 
            and transaction_sub_type = '%s' 
            and object_id = %s and object_type = '%s' 
            and `state` = '%s';
        ", UserTransaction::TRANSACTION_TYPE_ORDER_PAYMENT,
            UserTransaction::TRANSACTION_SUB_TYPE_ORDER_PAYMENT_SHIPPING_CHINA_VIETNAM,
            $order_id,
            UserTransaction::OBJECT_TYPE_ORDER,
            UserTransaction::STATE_COMPLETED));
        if($query){
